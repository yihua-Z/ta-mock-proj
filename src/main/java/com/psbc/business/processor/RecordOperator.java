...
public class RecordOperator {
    ...
    public AccountApplication recordToDatabase(TableModel tableModel, AccountApplication accountApplication) {
        HashMap<String, String[]> stringHashMap = null;

        List<Field> tableModelDeclaredFields = Arrays.asList(tableModel.getClass().getDeclaredFields());
        List<Field> databaseModelDeclaredFields = Arrays.asList(accountApplication.getClass().getDeclaredFields());

        for (Field a : tableModelDeclaredFields
        ) {
            Object value = invokeGetMethod(tableModel, a.getName(), null);
                ...
                if (a.getName().equals(c.getName())) {
                    try {
                        if (c.getType().getSimpleName().equals("String")) {
                            c.set(accountApplication, value.toString());
                        }
                        if (c.getType().getSimpleName().equals("Double")) {
                            c.set(accountApplication, Double.valueOf(value.toString()));
                        }
        ...
        for (Field c : databaseModelDeclaredFields) {
            ...
            try {
                c.setAccessible(true);
                o = c.get(accountApplication);
                if (o == null) {
                    c.set(accountApplication, "");
                }
            } catch (IllegalAccessException e) {
                e.printStackTrace();
            }
